...
const TableLocationList = ({ allLocation, handleDelete, pagination, setPagination }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return <div className="text-center">{row.getValue("detailLocation")}</div>;
      }
    },
    {
      ...
      cell: ({ row }) => {
        return <div className="text-center">{row.getValue("phoneNumber")}</div>;
      }
    },
    {
      accessorKey: "status",
  ...
  return (
    <Fragment>
      <div className="flex flex-col md:flex-row gap-4 md:gap-10 py-4">
        ...
        {/* Status tabs */}
        <Tabs
          defaultValue={pagination.statusLocation}
          className="w-full md:w-auto flex justify-center"
          onValueChange={(val) =>
            setPagination((prev) => ({ ...prev, statusLocation: val, page: 1 }))
          }>
      ...
      <div className="flex flex-row flex-wrap items-center justify-center gap-4 py-4">
        ...
        <div className="flex items-center gap-2 flex-1 justify-end">
          ...
          <span>
            Page {pagination.page} of {allLocation?.data?.pagination?.totalPages}
          </span>
          <Button
            variant="outline"
            size="sm"
            onClick={() => setPagination((prev) => ({ ...prev, page: prev.page + 1 }))}
            disabled={pagination.page === allLocation?.data?.pagination?.totalPages}>
            Next
          </Button>
        </div>
      </div>
