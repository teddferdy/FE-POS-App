...
const ProductList = () => {
  ...
  const TABLE_SHOW = useMemo(() => {
    ...
    if (allProduct.data && allProduct.isSuccess) {
      const columns = [
        {
          accessorKey: "image",
          header: ({ column }) => {
        ...
        },
        {
          ...
          cell: ({ row }) => (
            <div className="text-center capitalize">{row.getValue("nameProduct")}</div>
          )
        },
        {
        ...
        },
        {
          ...
          cell: ({ row }) => {
            return (
              <div className="text-center font-medium">{row.getValue("createdBy") || "-"}</div>
            );
          }
        },
        {
