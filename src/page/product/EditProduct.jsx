...
const EditProduct = () => {
  ...
  const handleSelectPriceTemplate = async (templateId) => {
    ...
    try {
      ...
      if (template?.tiers) {
        setPriceTiers(template.tiers.map((t) => ({ id: Date.now() + Math.random(), name: t.name, price: t.price || 0 })));
      }
    } catch {
              ...
              {[
                ...
                <React.Fragment key={s.step}>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm font-medium hidden sm:inline ${currentStep >= s.step ? "text-foreground" : "text-muted-foreground"}`}>
                      {s.label}
                    </span>
                  </div>
                  {i < 2 && (
                    <div className={`flex-1 h-0.5 mx-3 rounded transition-colors ${currentStep > s.step ? "bg-primary" : "bg-muted"}`} />
                  )}
                </React.Fragment>
              ))}
            </div>
            ...
            <div className="lg:col-span-2 space-y-6">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="md:col-span-2">
                        ...
                        <Textarea placeholder="Deskripsi produk (opsional)" rows={3} {...field} />
                        <FormMessage />
                      </FormItem>
                    )}
                  />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="mb-4">
                        ...
                        <Combobox
                          options={priceListTemplates.map((t) => ({ value: String(t.id), label: t.name }))}
                          value={field.value}
                          onChange={(value) => {
                ...
                <div className="space-y-3">
                  ...
                  {priceTiers.length === 0 && (
                    <p className="text-xs text-muted-foreground">
                      Tambahkan harga khusus untuk pelanggan grosir, member, atau tingkat lainnya.
                    </p>
                  )}
                </div>
              </Card>
              ...
              <Card className="p-6">
                <div className="flex items-center gap-2 pb-4 border-b border-border mb-5">
                  ...
                  <h3 className="text-base font-semibold text-foreground">Modifier / Tambahan</h3>
                </div>
                <div className="flex items-center justify-between">
                  <div>
              ...
              <Card className="p-6">
                ...
                <div className="space-y-3 text-xs text-muted-foreground">
                  <div className="flex items-start gap-2">
                    ...
                    <span>
                      SKU:{" "}
                      <strong className="text-foreground font-mono">{product.sku || "-"}</strong>
                    </span>
                  </div>
                  <div className="flex items-start gap-2">
                    <span className="material-symbols-outlined text-base text-primary shrink-0">
