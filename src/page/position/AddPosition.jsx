...
const AddPosition = () => {
  ...
  const mutation = useMutation((data) => addPosition(data), {
    onSuccess: () => {
      queryClient.invalidateQueries("positions");
      Toast.fire({ icon: "success", title: t("page.position.created") });
      navigate("/position");
    },
    onError: (err) => {
      Toast.fire({
        icon: "error",
        title: err?.response?.data?.message || t("page.position.createError")
      });
    }
  });
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-2xl mx-auto p-6">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-xl font-bold">{t("page.position.addTitle")}</h1>
            <p className="text-sm text-muted-foreground">{t("page.position.addDesc")}</p>
          </div>
        </div>
