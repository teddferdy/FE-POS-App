...
const AddCategory = () => {
  ...
  const mutation = useMutation((data) => addCategory(data), {
    onSuccess: () => {
      queryClient.invalidateQueries("categories");
      Toast.fire({ icon: "success", title: t("page.category.created") });
      navigate("/category");
    },
    onError: (err) => {
      Toast.fire({
        icon: "error",
        title: err?.response?.data?.message || t("page.category.createError")
      });
    }
  });
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-2xl mx-auto p-6">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-xl font-bold">{t("page.category.addTitle")}</h1>
            <p className="text-sm text-muted-foreground">{t("page.category.addDesc")}</p>
          </div>
        </div>
