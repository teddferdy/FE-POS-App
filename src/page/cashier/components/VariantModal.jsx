...
const VariantModal = ({ product, onSelect, onClose }) => {
  ...
  const titleKey = hasOptions ? "page.cashier.selectOption"
    : hasModifiers ? "page.cashier.selectModifier"
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          {hasVariants && !hasOptions && !hasModifiers && (
            <div className="space-y-2">
              ...
              {product.variant.filter(v => v != null && (v.nameVariant || v.name)).map((v, idx) => {
                const isSelected = selectedOption?.label === (v.nameVariant || v.name);
                const stock = v.stock != null ? Number(v.stock) : null;
                const outOfStock = stock !== null && stock <= 0;
                return (
                  <button key={v.nameVariant || idx}
                    onClick={() => !outOfStock && setSelectedOption(
                      selectedOption?.label === (v.nameVariant || v.name) ? null : { label: v.nameVariant || v.name, price: v.price, image: v.image }
                    )}
                    disabled={outOfStock}
                    className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-all duration-200 text-left ${
                      outOfStock ? "border-border/20 bg-muted/30 opacity-50 cursor-not-allowed"
                        : isSelected ? "border-primary bg-primary/10 shadow-sm shadow-primary/10"
                    ...
                    {v.image ? (
                      <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0 bg-muted/50">
                        <img src={optimizeImage(v.image) || "/placeholder.svg"} alt={v.nameVariant || v.name || ""} className="w-full h-full object-cover" />
                      </div>
                    ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {stock !== null && <p className={`text-xs ${outOfStock ? "text-destructive" : "text-muted-foreground/60"}`}>{t("page.cashier.stock")}: {stock}</p>}
                      </div>
                    </div>
                    {isSelected && <div className="w-6 h-6 rounded-full bg-primary flex items-center justify-center"><Check size={12} className="text-primary-foreground" /></div>}
                    {outOfStock && <PackageOpen size={16} className="text-destructive/60 shrink-0" />}
                  </button>
                );
              })}
            </div>
          ...
          {hasOptions && (
            <div className="space-y-3">
              <p className="text-sm font-semibold text-foreground border-b border-border/30 pb-1">Options</p>
              {product.options.map(group => (
                <div key={group.id || group.name}>
                  <p className="text-xs font-medium text-muted-foreground mb-1.5">{group.name}</p>
                  <div className="space-y-1.5 pl-2">
                    {group.options?.filter(o => o.name).map((opt, idx) => {
                      const isSelected = selectedOption?.label === `${group.name} - ${opt.name}`;
                      const stock = opt.stock != null ? Number(opt.stock) : null;
                      const outOfStock = stock !== null && stock <= 0;
                      return (
                        <button key={opt.name || idx}
                          onClick={() => !outOfStock && setSelectedOption(
                            selectedOption?.label === `${group.name} - ${opt.name}` ? null : { label: `${group.name} - ${opt.name}`, price: opt.price, stock: opt.stock }
                          )}
                          disabled={outOfStock}
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2">
                              <p className="text-xs text-muted-foreground">+Rp {formatPrice(opt.price)}</p>
                              {stock !== null && <p className={`text-xs ${outOfStock ? "text-destructive" : "text-muted-foreground/60"}`}>{t("page.cashier.stock")}: {stock}</p>}
                            </div>
                          </div>
                          {isSelected && <Check size={14} className="text-primary shrink-0" />}
                          {outOfStock && <PackageOpen size={14} className="text-destructive/60 shrink-0" />}
                        </button>
                      );
                    })}
                  </div>
          ...
          {hasModifiers && (
            <div className="space-y-2">
              <p className="text-sm font-semibold text-foreground border-b border-border/30 pb-1">Modifiers</p>
              {product.modifiers.filter(m => m.name).map((m, idx) => {
                const isSelected = selectedModifier?.label === m.name;
                const stock = m.stock != null ? Number(m.stock) : null;
                const outOfStock = stock !== null && stock <= 0;
                return (
                  <button key={m.id || m.name || idx}
                    onClick={() => !outOfStock && setSelectedModifier(selectedModifier?.label === m.name ? null : { label: m.name, price: m.price, stock: m.stock })}
                    disabled={outOfStock}
                    className={`w-full flex items-center gap-3 p-2.5 rounded-lg border transition-all text-left ${
                      outOfStock ? "border-border/20 bg-muted/30 opacity-50 cursor-not-allowed"
                        : isSelected ? "border-primary bg-primary/10"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        <p className="text-xs text-muted-foreground">+Rp {formatPrice(m.price)}</p>
                        {stock !== null && <p className={`text-xs ${outOfStock ? "text-destructive" : "text-muted-foreground/60"}`}>{t("page.cashier.stock")}: {stock}</p>}
                      </div>
                    </div>
                    {outOfStock && <PackageOpen size={14} className="text-destructive/60 shrink-0" />}
