...
const EditBundle = () => {
  ...
  const {
    ...
    isError
  } = useQuery(["bundle", id], () => getBundleById(id), { enabled: !!id });
  ...
  useEffect(() => {
    if (bundleData?.data) {
      ...
      let fromTime = "00:00";
      if (b.validFrom) {
        const d = new Date(b.validFrom);
        if (!isNaN(d.getTime())) {
          fromDate = d;
          fromTime = format(d, "HH:mm");
        }
      }
      let untilDate = null;
      let untilTime = "23:59";
      if (b.validUntil) {
        ...
        if (!isNaN(d.getTime())) {
          untilDate = d;
          untilTime = format(d, "HH:mm");
        }
      }
      setValidFromDate(fromDate);
      setValidFromTime(fromTime);
      setValidUntilDate(untilDate);
      setValidUntilTime(untilTime);

      const storeValue = Array.isArray(b.store) ? (b.store[0] ?? null) : b.store;
      setFormData({
        name: b.name || "",
      ...
      // ponytail: draft dianggap aktif — status akhir ditentukan toggle / tombol draft
      setIsActive(b.status !== "inactive");

      if (storeValue) {
        setSelectedStores([storeValue]);
        setAllStores(false);
      } else {
        setAllStores(true);
      }
  ...
  const updateMutation = useMutation((payload) => updateBundle(id, payload), {
    onSuccess: () => {
  ...
  const buildPayload = () => ({
    ...formData,
    image: imageRemoved ? null : formData.image,
  ...
  if (isLoadingBundle) return <Loading />;
  if (isError || !bundleData?.data) return <AbortController refetch={() => {}} />;

  const shownImage = previewImage || (imageRemoved ? null : formData.image);
      ...
      <div className="space-y-6 pb-4">
        <PageHeader
          breadcrumbs={[
            ...
            { label: t("page.bundle.editTitle"), i18nKey: "page.bundle.editTitle" }
          ]}
          title={t("page.bundle.editTitle")}
          description={t("page.bundle.editDescription")}
          backLink="/bundle"
                          ...
                          <div
                            ...
                            <div className="flex-1 space-y-1">
                              <Label className="text-xs">
                                {t("page.bundle.form.selectProduct")}
                              </Label>
                              <Select
                                value={item.product}
