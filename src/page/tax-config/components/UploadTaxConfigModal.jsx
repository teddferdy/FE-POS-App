...
const UploadTaxConfigModal = ({ open, onOpenChange, onUploadSuccess }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await uploadTaxConfigExcel(file);
      setUploadStatus("success");
      toast.success(t("common.success"), {
        description: res.message || t("page.taxConfig.toast.uploadSuccess")
      });
      setTimeout(() => {
        handleClose();
      ...
      const msg =
        ...
        err?.message ||
        t("page.taxConfig.toast.uploadError");
      ...
      <div className="relative bg-background rounded-xl border border-border shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-5 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold">{t("page.taxConfig.upload.title")}</h2>
              <p className="text-xs text-muted-foreground">{t("page.taxConfig.upload.description")}</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="p-5 space-y-5">
          <div className="flex items-start gap-3 p-4 rounded-lg bg-muted/50 border border-border">
            ...
            <div className="text-sm text-muted-foreground space-y-1">
              <p>{t("page.taxConfig.upload.formatHint")}</p>
              <p className="text-xs">{t("page.taxConfig.upload.columnsHint")}</p>
            </div>
          </div>
...
UploadTaxConfigModal.propTypes = {
  open: PropTypes.bool.isRequired,
  onOpenChange: PropTypes.func.isRequired,
  onUploadSuccess: PropTypes.func
};

export default UploadTaxConfigModal;
