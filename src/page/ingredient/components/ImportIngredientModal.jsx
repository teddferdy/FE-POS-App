...
const ImportIngredientModal = ({ onClose }) => {
  ...
  const mutation = useMutation((data) => uploadIngredientExcel(data), {
    onSuccess: () => {
      queryClient.invalidateQueries("ingredients");
      Toast.fire({ icon: "success", title: t("page.ingredient.importSuccess") });
      onClose();
    },
    onError: (err) => {
      Toast.fire({
        icon: "error",
        title: err?.response?.data?.message || t("page.ingredient.importError")
      });
    }
  });
  ...
  const handleUpload = () => {
    if (!file) {
      Toast.fire({ icon: "warning", title: t("page.ingredient.selectFileFirst") });
      return;
    }
      ...
      <div className="bg-card/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-border/50 w-full max-w-lg overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border/50">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold">{t("page.ingredient.importTitle")}</h2>
          </div>
        ...
        <div className="p-6 space-y-5">
          <div
            ...
            {file ? (
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-sm text-muted-foreground">{t("page.ingredient.dragDrop")}</p>
                <p className="text-xs text-muted-foreground/60">
                  {t("page.ingredient.supportedFormats")}
                </p>
              </div>
            )}
            <input
          ...
          <a
            href="/templates/ingredient-template.xlsx"
            download
            className="flex items-center gap-2 text-sm text-primary hover:text-primary/80 transition-colors">
            <Download size={14} />
            {t("page.ingredient.downloadTemplate")}
          </a>

          {file && (
            <div className="flex items-start gap-2 p-3 rounded-lg bg-muted/30 border border-border/40">
              <AlertCircle size={14} className="text-amber-500 mt-0.5 shrink-0" />
              <p className="text-xs text-muted-foreground">{t("page.ingredient.importNote")}</p>
            </div>
          )}
