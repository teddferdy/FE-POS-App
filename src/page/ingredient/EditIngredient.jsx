...
const EditIngredient = () => {
  ...
  const { isLoading } = useQuery(["ingredient", id], () => getIngredientById(id), {
    enabled: !!id,
    onSuccess: (res) => {
      const ing = res?.data || res;
      setForm({
        nameIngredient: ing.nameIngredient || ing.name || "",
        type: ing.type || "",
        desc: ing.desc || ing.description || ""
      });
    }
  });
  ...
  const mutation = useMutation((data) => editIngredient(id, data), {
    onSuccess: () => {
      queryClient.invalidateQueries("ingredients");
      Toast.fire({ icon: "success", title: t("page.ingredient.updated") });
      navigate("/ingredient");
    },
    onError: (err) => {
      Toast.fire({
        icon: "error",
        title: err?.response?.data?.message || t("page.ingredient.updateError")
      });
    }
  });
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-2xl mx-auto p-6">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-xl font-bold">{t("page.ingredient.editTitle")}</h1>
            <p className="text-sm text-muted-foreground">{t("page.ingredient.editDesc")}</p>
          </div>
        </div>
